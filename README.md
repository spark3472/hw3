...
The signals that the shell ignores are SIGQUIT, SIGINT, SIGTSTP, SIGTTIN, and SIGTTOU. However, these signals are reset for the child processes. The shell handles are SIGCHLD and in order to determine when a child process has finished running so that it can be removed from the job list. The shell hands SIGCHLDs by masking them around areas of critical regions, namely removing or adding to the job list by using sigprogmask.
